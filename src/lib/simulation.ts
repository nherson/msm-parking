...
export class Simulator {
  ...
  run(times: number): SimulationSummary {
    ...
    for (const block of BLOCKS) {
      for (const result of results) {
        availabilityPerBlock[block] += result.mainStreetParking.availabilityOnBlock(block)
      }
    }
    BLOCKS.forEach((b) => availabilityPerBlock[b] = (availabilityPerBlock[b] / results.length)) // normalize

    return {
      data: availabilityPerBlock,
      oddsOfParkingAvailableOnBlock: (results.filter(item => item.canParkOnTargetBlock).length / results.length),
      oddsOfParkingAvailableOnAdjacentBlock: (results.filter(item => item.canParkOnAdjacentBlock).length / results.length)
    }
  }
  ...
  runOnce(): SimulationRunResult {
    ...
    return {
      mainStreetParking: mainStreetParking,
      canParkOnTargetBlock: mainStreetParking.availabilityOnBlock(targetBlock) > 0,
      canParkOnAdjacentBlock: mainStreetParking.availabilityOnAdjacentBlocks(targetBlock) > 0
    }
  }
